...
class Unused
    ...
    def find_usages_in_files(files, xibs, items_in)
      ...
      files.each { |file|
        file_content = File.read(file, encoding: 'utf-8', invalid: :replace, replace: '?')

        if !file_content.valid_encoding?
          file_content = file_content.encode('UTF-16be', :invalid => :replace, :replace => '?').encode('UTF-8')
        end

        lines = file_content.lines.map { |line| line.gsub(/^[^\/]*\/\/.*/, '') }
        words = lines.join("\n").split(/\W+/)
        words_array = words.group_by { |w| w }.map { |w, ws| [w, ws.length] }.flatten
        ...
        # Remove all items which have usage 2+
    ...
    def grab_items(file)
        ...
        lines = file_content.lines.map { |line| line.gsub(/^\s*\/\/.*/, "") }
